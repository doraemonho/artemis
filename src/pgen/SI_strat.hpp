  ...
  if (!(params.hasKey("SI_strat_params"))) {
    ...
    if (strat_params.StratFlag) {
      ...
      if (do_dust) {
        ...
        auto Hdust_h = strat_params.Hdust.GetHostMirror();
	auto Hdust_v = pin->GetVector<Real>("dust", "Hratio");
        for (int n = 0; n < nDust; n++) {
          Hdust_h(n) = strat_params.h * Hdust_v[n];
        }
        strat_params.Hdust.DeepCopy(Hdust_h);
      }
    }
...
inline void ProblemGenerator(MeshBlock *pmb, ParameterInput *pin) {
  ...
  PARTHENON_REQUIRE(do_dust, "SI_strat pgen requires dust=true!");
  PARTHENON_REQUIRE(const_stopping_time, "SI_strat pgen requires const_stopping_time=true!");
  PARTHENON_REQUIRE(strat_params.ShBoxCoord == 2, "SI_strat pgen requires shboxCoord=2!");
  PARTHENON_REQUIRE(pmb->pmy_mesh->ndim == 2, "SI_strat pgen requires ndim = 2!");
...
inline void ExtrapInnerX1(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  auto &pars = strat_params;
  pmb->par_for_bndry(
      "SIStratInnerX1", nb, IndexDomain::inner_x1, parthenon::TopologicalElement::CC,
      coarse, fine,
...
//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator SI_strat::ExtrapOuterX1()
//! \brief Sets BCs on +x boundary in shearing box
template <Coordinates GEOM>
inline void ExtrapOuterX1(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  auto &pars = strat_params;
  pmb->par_for_bndry(
      "SIStratOuterX1", nb, IndexDomain::outer_x1, parthenon::TopologicalElement::CC,
      coarse, fine,
      KOKKOS_LAMBDA(const int &l, const int &k, const int &j, const int &i) {
	v(0, gas::prim::velocity(2), k, j, i) -= pars.qomL;
...
//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator SI_strat::ExtrapInnerX2()
...
inline void ExtrapInnerX2(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  pmb->par_for_bndry(
      "SIStratInnerX2", nb, IndexDomain::inner_x2, parthenon::TopologicalElement::CC,
      coarse, fine,
      KOKKOS_LAMBDA(const int &l, const int &k, const int &j, const int &i) {
        // Extract coordinates
        geometry::Coords<GEOM> coords(pco, k, j, i);
...
//----------------------------------------------------------------------------------------
//! \fn void ProblemGenerator SI_strat::ExtrapOuterX2()
//! \brief Sets BCs on +y boundary in SI shearing box
template <Coordinates GEOM>
inline void ExtrapOuterX2(std::shared_ptr<MeshBlockData<Real>> &mbd, bool coarse) {
  ...
  pmb->par_for_bndry(
      "SIStratOuterX2", nb, IndexDomain::outer_x2, parthenon::TopologicalElement::CC,
      coarse, fine,
      KOKKOS_LAMBDA(const int &l, const int &k, const int &j, const int &i) {
        // Extract coordinates
        geometry::Coords<GEOM> coords(pco, k, j, i);
